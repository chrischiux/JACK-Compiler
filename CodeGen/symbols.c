...
void NewMethod(char *name, char *type)
{
	...
	strcpy(currentClass->methods[currentClass->methodCount].type, type);
...
int InsertMethodSymbol(char *name, char *type, Kinds kind)
{
	...
	if (kind == arg)
	{
		currentMethod->symbols[currentMethod->count].index = currentMethod->argCount;
		currentMethod->argCount++;
	}
	else if (kind == var)
	{
		currentMethod->symbols[currentMethod->count].index = currentMethod->varCount;
...
char *getSubroutineType(char *class, char *method)
{
	for (int i = 0; i < classCount; i++)
	{
		if (strcmp(class, table[i].name) == 0)
		{
			...
			{
				if (strcmp(method, table[i].methods[j].name) == 0)
				{
					return table[i].methods[j].type;
				}
			}
		}
	}
...
{
	return currentClass->fieldCount;
}

int getCurrentMethodVarCount()
{
	return currentMethod->varCount;
}
...
Kinds getKind(Token t)
{
	for (int i = 0; i < currentMethod->count; i++)
	{
		if (strcmp(t.lx, currentMethod->symbols[i].name) == 0)
		{
			return currentMethod->symbols[i].kind;
		}
	}
	for (int i = 0; i < currentClass->symbolCount; i++)
	{
		if (strcmp(t.lx, currentClass->symbols[i].name) == 0)
		{
			return currentClass->symbols[i].kind;
		}
	}
	return -1;
}
...
int getIndexOf(Token t)
{
	for (int i = 0; i < currentMethod->count; i++)
	{
		if (strcmp(t.lx, currentMethod->symbols[i].name) == 0)
		{
			return currentMethod->symbols[i].index;
		}
	}
	for (int i = 0; i < currentClass->symbolCount; i++)
	{
		if (strcmp(t.lx, currentClass->symbols[i].name) == 0)
		{
			return currentClass->symbols[i].index;
		}
	}
	return -1;
}
...
void printAll(){
	for (int i = 0; i < classCount; i++)
	{
		...
		for (int j = 0; j < table[i].methodCount; j++)
		{
			...
			{
				printf("%s, %s, %d, index: %i\n", table[i].methods[j].symbols[k].name, table[i].methods[j].symbols[k].type, table[i].methods[j].symbols[k].kind, table[i].methods[j].symbols[k].index);
			}
		}
	}
}
